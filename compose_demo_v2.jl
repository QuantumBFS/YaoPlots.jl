...
plot(chain(chain( 4, put(1=>X), repeat(H, 2:4), control(2, 1=>X), control(4, 3=>X), control(3, 1=>X), control(4, 3=>X), repeat(H, 1:4), )))
"""
...
# TODO: unfilled circle 
# S-gate draw a cross inside a circle 
...
Compose.compose!(ctx::Context, x::MGate) = square(ctx, "M") # Measurement [M symbol]
...
# TODO: print Ket notation for the input qubits for Q_0 to Q_n where n = number of qubits in circuit  [IGNORE THIS for now ]
...
# functoin for control block 

function Compose.compose!(ctx::Context, blk::ControlBlock)

# measurement 


# Do code review after
end # module
